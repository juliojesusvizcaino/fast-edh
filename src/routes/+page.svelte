<script lang="ts">
	...
	const layouts = {
		...
		[3]: {
			...
			players: [
				{ rotation: 90, gridArea: '1 / 1 / 8 / 3' }, // Bottom-Left
				{ rotation: 180, gridArea: '1 / 3 / 4 / 8' }, // Bottom-Right
				{ rotation: 0, gridArea: '4 / 3 / 8 / 8' } // Top-Center
			]
		},
		[4]: {
			gridClass: 'grid grid-cols-2 grid-rows-2',
	...
	let playerCount: 2 | 3 | 4 | 5 = $state(3);
	let players = $derived(
		Array.from(
			{ length: playerCount },
			(_, i) =>
...
<svelte:document
	...
	onfullscreenchange={handleVisibilityChange}
/>

<div
	class={[
		'fixed z-10 flex -translate-1/2 flex-col gap-2',
		playerCount === 4 && 'top-1/2 left-1/2',
		playerCount === 3 && 'top-1/2 left-1/3'
...
>
	...
	>
		<Dialog.Trigger
			class="size-12 cursor-pointer items-center justify-center rounded-full bg-neutral-800 p-2 text-white"
		>
			{playerCount}
		</Dialog.Trigger>
		<Dialog.Portal>
			...
			<Dialog.Content
				class="
          shadow-popover data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0
          data-[state=closed]:zoom-out-95 data-[state=open]:zoom-in-95 fixed top-[50%] left-[50%] z-50 flex translate-x-[-50%] translate-y-[-50%]
          flex-col gap-2 rounded-2xl bg-neutral-700 px-16 py-8 text-white outline-hidden
        "
			>
				{#each [3, 4] as const as i (i)}
					<button
						class="min-w-32 rounded-2xl bg-neutral-600 p-4"
						onclick={() => {
							playerCount = i;
						}}
					>
